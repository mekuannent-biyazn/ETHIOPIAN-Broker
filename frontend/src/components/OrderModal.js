...
const OrderModal = ({ property, isOpen, onClose, onOrderSuccess }) => {
  ...
  const [paymentFlow, setPaymentFlow] = useState("immediate"); // Add payment flow state
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await orderProperty(property._id, {
        paymentMethod,
        extraInfo,
        paymentFlow, // Include payment flow in the request
      });

      // Handle immediate payment redirect
      if (result.paymentFlow === "immediate" && result.paymentUrl) {
        // Redirect to payment URL for immediate payment
            ...
            <div className="space-y-2">
              <label className="flex items-center">
                <input
                  ...
                  name="paymentFlow"
                  value="immediate"
                  checked={paymentFlow === "immediate"}
                  onChange={(e) => setPaymentFlow(e.target.value)}
                  className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                />
                <span className="ml-2 text-sm text-gray-900">
                  <strong>Pay Now</strong> - Complete payment immediately after order
                </span>
              </label>
              <label className="flex items-center">
                <input
                  ...
                  name="paymentFlow"
                  value="later"
                  checked={paymentFlow === "later"}
                  onChange={(e) => setPaymentFlow(e.target.value)}
                  className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                />
                <span className="ml-2 text-sm text-gray-900">
                  <strong>Pay Later</strong> - Complete payment within 3 days (72 hours)
                </span>
              </label>
            </div>
          </div>
