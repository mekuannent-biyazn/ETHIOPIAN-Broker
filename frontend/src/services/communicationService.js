...
const communicationService = {
  ...
  // Enhanced file download with multiple methods
  downloadFileEnhanced: async (messageId, filename = null) => {
    try {
      // Method 1: Try direct file access
      const response = await axios.get(`/api/communication/file/${messageId}`, {
        responseType: "blob",
      });
      ...
      console.error("Enhanced download method 1 failed:", error);

      // Method 2: Fallback to original download
      try {
        const response = await axios.get(
          `/api/communication/download/${messageId}/file`,
          {
        ...
        console.error("Enhanced download method 2 failed:", fallbackError);

        // Method 3: Open in new tab as last resort
        try {
          window.open(`/api/communication/file/${messageId}`, "_blank");
          return true;
        } catch (finalError) {
  ...
  getFilePreviewUrl: (messageId) => {
    return `/api/communication/file/${messageId}`;
  },
