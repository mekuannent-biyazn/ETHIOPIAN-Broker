...
const communicationReducer = (state, action) => {
  switch (action.type) {
    ...
    case "ADD_MESSAGE":
      ...
      const updatedActiveConversation = shouldAddToActive
        ? {
            ...state.activeConversation,
...
const CommunicationProvider = ({ children }) => {
  ...
  const sendMessage = useCallback(
    async (messageData) => {
      try {
        // Create optimistic message with proper ID
        ...
        const optimisticMessage = {
          ...
          receiver: {
            ...
            fname: "", // Will be populated by backend
            lname: "",
            role: "",
          },
          content: messageData.content,
          messageType: messageData.file
          ...
          fileSize: messageData.file?.size || null,
          fileType: messageData.file?.type || null,
        ...
        if (socketService.getConnectionStatus()) {
          socketService.emit("newMessage", response.data);
        }
