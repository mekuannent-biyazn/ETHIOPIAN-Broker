...
const UserSearch = ({ onUserSelect }) => {
  ...
  const searchUsers = async (query) => {
    ...
    try {
      ...
      if (filteredUsers.length === 0 && allUsers.length > 0) {
        setError(t("communication.noUsersFound"));
  ...
  const getCommunicationHint = () => {
    ...
    return t("communication.searchHint");
  };
  ...
  return (
    <div className="relative mb-4" dir={language === "am" ? "rtl" : "ltr"}>
      <div className="relative">
        <svg
          className={`absolute ${language === "am" ? "right-3" : "left-3"} top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        <input
          ...
          className={`w-full ${language === "am" ? "pr-10 pl-3" : "pl-10 pr-3"} py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent`}
        ...
        {searchTerm && (
          <button
            onClick={clearSearch}
            className={`absolute ${language === "am" ? "left-3" : "right-3"} top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors`}
          >
            <svg
              className="w-4 h-4"
              fill="none"
      ...
      {/* Communication Hint */}
      <div
        className="mt-1 text-xs text-gray-500 flex items-center space-x-1"
          ...
          {loading ? (
            ...
            <div className="p-4 text-center text-gray-500">
              <div className="flex flex-col items-center space-y-2">
                ...
                <p className="text-xs">
                  {t("communication.tryDifferentSearch")}
                </p>
                <div className="text-xs text-gray-400 mt-2">
                  <p>
                    {t("communication.availableUsersCount", {
                      count: allUsers.length,
                    })}
                  </p>
                  <p>
                    {t("communication.yourRole")}:{" "}
