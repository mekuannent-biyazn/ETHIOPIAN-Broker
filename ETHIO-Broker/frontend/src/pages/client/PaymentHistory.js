...
const PaymentHistory = () => {
  ...
  const [filters, setFilters] = useState({
    status: "Completed", // Default to only Completed
    paymentType: "",
    page: 1,
    limit: 10,
  });
  ...
  const fetchPayments = async () => {
    try {
      ...
      if (response.data.success) {
        // Filter to only show completed payments on the frontend as well
        const completedPayments = response.data.data.payments.filter(
        ...
        setPagination({
          ...
          totalPages: Math.ceil(completedPayments.length / filters.limit),
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            {t("client.viewCompletedTransactions")}
          </p>
        </div>

        {/* Filters - Updated to only show Completed status */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <select
                ...
                disabled // Disabled since we only show Completed
              >
                <option value="Completed">{t("client.completed")}</option>
              </select>
              <p className="text-xs text-gray-500 mt-1">
                {t("client.onlyCompletedVisible")}
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
        ...
        {/* Payments List - Only showing completed */}
        {payments.length === 0 ? (
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            <div className="text-6xl mb-4">✅</div>
            <h3 className="text-xl font-semibold text-gray-900 mb-2">
              {t("client.noCompletedPayments")}
            </h3>
            <p className="text-gray-600 mb-4">
              {filters.paymentType
                ? t("client.noCompletedPaymentsMatchType")
                : t("client.noCompletedPaymentsYet")}
            </p>
            <Link
              to="/properties"
        ...
        {/* Summary - Only showing completed statistics */}
        <div className="mt-8 bg-white p-6 rounded-lg shadow-md">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">
            {t("client.completedPaymentsSummary")}
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div className="text-center p-4 bg-green-50 rounded-lg border border-green-200">
              ...
              <div className="text-green-700">{t("client.totalCompleted")}</div>
            </div>
                ...
                {formatPrice(
                  payments.reduce((sum, p) => {
                    const totalCommission =
                      (p.buyerCommission || 0) + (p.sellerCommission || 0);
                    return sum + totalCommission;
                  }, 0),
                )}
              </div>
              <div className="text-purple-700">
                {t("client.totalCommission")}
              </div>
            </div>
          </div>
        </div>
